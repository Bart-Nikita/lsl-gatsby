...
const TrainingsSteps = () => {
    ...
    useLayoutEffect(() => {
        if (decorItem?.current && listItem?.current) {
            ...
            const fullWith = width < 768 ? listItem.current.getBoundingClientRect().height : listItem.current.getBoundingClientRect().width
            const itemWith = decorItem.current.getBoundingClientRect().width
            const int = Math.floor(fullWith / (itemWith + 10))
            setInt(int)
        }
    }, [decorItem, listItem, isInView, width])
    ...
    return (
        <section className="container-new mb-[140px] xl:mb-[92px] md:mb-[64px]">
            <h2 className={'text-large-new mb-[52px] xl:mb-[17px] md:mb-[20px]'} dangerouslySetInnerHTML={{__html: trainingsPage?.wpPage?.trainings?.trainingsStepsZagolovok || ''}}></h2>
            <ul className={'grid grid-cols-3 gap-y-[49px] gap-x-[28px] xl:grid-cols-2 xl:gap-y-[28px] xl:gap-x-[20px] md:grid-cols-1 md:gap-y-[13px]'}>
                {trainingsPage?.wpPage?.trainings?.trainingsStepsList?.map((item,index) =>
                    <li  ref={index === 0 ? listItem : null} key={index} className={'md:flex md:gap-[10px]'}>
                        <div className={'relative flex items-center gap-[68px] mb-[30px] xl:gap-[19px] xl:mb-[10px] md:mb-[0px] md:flex-row md:gap-[0px] '}>
                            ...
                            {index + 1 !== trainingsPage?.wpPage?.trainings?.trainingsStepsList?.length &&
                ...
                <li className={'md:flex md:gap-[10px] md:items-center'}>
                    ...
                    <p className={'text-[24px] text-[#515151] mr-[-100px] xl:mr-[0px] xl:text-black leading-[1.4] font-light xl:text-[16px] '}>{typo.execute(trainingsPage?.wpPage?.trainings?.trainingsStepsRemarka || '')}</p>
                </li>
            </ul>
        </section>
    );
